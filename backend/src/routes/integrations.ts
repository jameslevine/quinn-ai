...
// Separate router for Gmail callback (mounted before auth middleware)
export const gmailCallbackRouter: Router = Router();

// Gmail OAuth - Get authorization URL (MUST be before /:type route)
...
// This is on the gmailCallbackRouter which is mounted before auth middleware
gmailCallbackRouter.get("/callback", async (req: Request, res: Response) => {
  try {
...
// Get single integration by type (MUST be after specific routes like /gmail/auth)
